...
func (iq *IPQuery) Unique(unique bool) *IPQuery {
	iq.unique = &unique
	return iq
}
...
func (iq *IPQuery) QueryQueries() *AppQueryQuery {
	query := &AppQueryQuery{config: iq.config}
...
func (iq *IPQuery) Clone() *IPQuery {
	...
	return &IPQuery{
		...
		withQueries: iq.withQueries.Clone(),
...
func (iq *IPQuery) WithQueries(opts ...func(*AppQueryQuery)) *IPQuery {
	query := &AppQueryQuery{config: iq.config}
...
//		Scan(ctx, &v)
//
func (iq *IPQuery) Select(fields ...string) *IPSelect {
...
func (iq *IPQuery) sqlAll(ctx context.Context) ([]*IP, error) {
	...
	if query := iq.withQueries; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Queries = []*AppQuery{}
		}
		query.withFKs = true
		query.Where(predicate.AppQuery(func(s *sql.Selector) {
