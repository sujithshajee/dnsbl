...
// Where appends a list predicates to the IPUpdate builder.
...
func (iu *IPUpdate) AddQueries(a ...*AppQuery) *IPUpdate {
...
// ClearQueries clears all "queries" edges to the AppQuery entity.
func (iu *IPUpdate) ClearQueries() *IPUpdate {
	iu.mutation.ClearQueries()
	return iu
}

// RemoveQueryIDs removes the "queries" edge to AppQuery entities by IDs.
func (iu *IPUpdate) RemoveQueryIDs(ids ...uuid.UUID) *IPUpdate {
	iu.mutation.RemoveQueryIDs(ids...)
	return iu
}

// RemoveQueries removes "queries" edges to AppQuery entities.
func (iu *IPUpdate) RemoveQueries(a ...*AppQuery) *IPUpdate {
	ids := make([]uuid.UUID, len(a))
	for i := range a {
		ids[i] = a[i].ID
	}
...
// AddQueries adds the "queries" edges to the AppQuery entity.
func (iuo *IPUpdateOne) AddQueries(a ...*AppQuery) *IPUpdateOne {
	ids := make([]uuid.UUID, len(a))
	for i := range a {
		ids[i] = a[i].ID
	}
...
// RemoveQueries removes "queries" edges to AppQuery entities.
func (iuo *IPUpdateOne) RemoveQueries(a ...*AppQuery) *IPUpdateOne {
	ids := make([]uuid.UUID, len(a))
	for i := range a {
		ids[i] = a[i].ID
	}
...
func (iuo *IPUpdateOne) Select(field string, fields ...string) *IPUpdateOne {
